...
#         """
#         if len(history) < 2:
#             return {'pattern': 'new_learner', 'trend': 'stable', 'struggling_topics': []}
        
#         recent_scores = [quiz['accuracy'] for quiz in history[-5:]]  # Last 5 quizzes
        
#         # Determine learning trend
#         if len(recent_scores) >= 3:
#             recent_trend = np.mean(recent_scores[-3:]) - np.mean(recent_scores[-5:-3] or recent_scores[:-3])
#             if recent_trend > 0.1:
#                 trend = 'improving'
...
#         current_topic = last_quiz['topic']
#         current_difficulty = last_quiz['difficulty']
#         current_score = last_quiz['accuracy']
#         current_num_questions = last_quiz['total']
...
#         # Count consecutive struggles in the same topic/difficulty
#         consecutive_struggles = 0
#         for quiz in reversed(history):
#             if quiz['topic'] == current_topic and quiz['difficulty'] == 'easy' and quiz['accuracy'] < 0.4:
#                 consecutive_struggles += 1
#             else:
#                 break
        
#         # Drastically reduce questions if struggling repeatedly
...
#         # If struggling in multiple topics, switch to their best topic for confidence
#         if len(analysis['struggling_topics']) > 2 and len(history) >= 3:
#             # Find their best performing topic
#             topic_scores = {}
#             for quiz in history:
#                 topic = quiz['topic']
...
#         # Move to medium difficulty if currently on easy
...
#         # If already on medium, continue with medium but more questions
#         return {
#             'topic': current_topic,
#             'difficulty': 'medium',
#             'num_questions': new_num_questions,
...
#         # If mastered hard level, suggest new topic
#         else:  # current_difficulty == 'hard'
#             next_topics = self.topic_relations.get(current_topic, [])
#             if not next_topics:
#                 # If no related topics, choose a random unmastered topic
#                 mastered_topics = set()
#                 for quiz in history:
...
#         total_questions = sum(quiz['total'] for quiz in history)
#         total_correct = sum(quiz['correct'] for quiz in history)
...
#     def _calculate_session_features(self, performance_history: List[Dict]) -> Dict:
#         """Calculates aggregated features from the user's recent quiz history."""
#         if not performance_history:
#             return {}
        
#         recent_sessions = performance_history[-3:]  # Last 3 sessions
